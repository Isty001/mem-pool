...
MU_TEST(test_pool)
{
    ...
    mu_assert(pool_fixed_is_allocated(pool, ptr1), "This should be known by the pool");
    mu_assert(pool_fixed_is_allocated(pool, ptr3), "This should be known by the pool");
    ...
    void *unknown = NULL;
    mu_assert(false == pool_fixed_is_allocated(pool, unknown), "This shouldn't be known by the Pool");
